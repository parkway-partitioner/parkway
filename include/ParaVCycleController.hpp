...
public:
  ...
  ParaVCycleController(ParaRestrCoarsener &rc, ParaCoarsener& c, ParaRefiner& r, SeqController& ref, int rank, int nP, int percentile, int inc, int approxRef, int limit, double limitAsPercent, ostream &out);
  ...
  void projectVCyclePartition(ParaHypergraph &cG, ParaHypergraph &fG, MPI_Comm comm);
  void shuffleVCycleVertsByPartition(ParaHypergraph &h, MPI_Comm comm);
  //void randomVCycleVertShuffle(ParaHypergraph &h, ParaHypergraph &fineH, MPI_Comm comm);
