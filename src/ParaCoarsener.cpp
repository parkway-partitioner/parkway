...
ParaHypergraph *ParaCoarsener::contractHyperedges(ParaHypergraph &h, MPI_Comm comm)
{
  register int i;
  ...
  if(myRank != numProcs-1) numMyClusters = clustersPerProc;
  else numMyClusters = clustersPerProc + Mod(totalClusters,numProcs);
  
  FastDynaArray<int> minClusterIndex(numProcs);
  FastDynaArray<int> maxClusterIndex(numProcs);
  FastDynaArray<int> *clusterWts = new FastDynaArray<int>(numMyClusters);
  ...
  for (i=0;i<numProcs;++i) 
      ...
      else 
	{
	  ...
	  if(i == numProcs-1) maxClusterIndex[i] = totalClusters;
	  else maxClusterIndex[i] = minClusterIndex[i] + clustersPerProc;
	}
    }
  ...
  for (i=0;i<numProcs;++i) 
    {
      ...
      sendLens[i] = max(clusterIndex-(max(clusterIndex+myMinCluIndex-maxClusterIndex[i],0)+max(minClusterIndex[i]-myMinCluIndex,0)),0);
    }
...
#  endif  
  ...
  MPI_Alltoallv(clusterWeights.getArray(), sendLens.getArray(), sendDispls.getArray(), MPI_INT, clusterWts->getArray(), recvLens.getArray(), recvDispls.getArray(), MPI_INT, comm); 

  myMinCluIndex = minClusterIndex[myRank];

  ParaHypergraph *coarseGraph = new ParaHypergraph(myRank,numProcs,numMyClusters,totalClusters,myMinCluIndex,stopCoarsening,clusterWts->getArray());
  ...
  if(dispOption > 1)
    {
      ...
      MPI_Reduce(&numLocHedges, &numTotCoarseHedges, 1, MPI_INT, MPI_SUM, 0, comm);
      MPI_Reduce(&numLocPins, &numTotCoarsePins, 1, MPI_INT, MPI_SUM, 0, comm);
      
      if(myRank == 0)
	{
	  out_stream << numTotCoarseVerts << " "
		     << numTotCoarseHedges << " " 
