...
parallel_restrictive_coarsening::parallel_restrictive_coarsening(int rank, int nProcs, int nParts,
                                       std::ostream &out)
  ...
  cluster_weights_ = nullptr;
  oartition_vector_ = nullptr;
}

parallel_restrictive_coarsening::~parallel_restrictive_coarsening() {}

void parallel_restrictive_coarsening::load(const parallel::hypergraph &h,
                              MPI_Comm comm) {
  int i;
  int endOffset;
  int startOffset;
...
parallel::hypergraph *parallel_restrictive_coarsening::contract_hyperedges(
    parallel::hypergraph &h,
    MPI_Comm comm) {
  parallel::hypergraph *coarseGraph =
      new parallel::hypergraph(rank_, processors_, cluster_index_,
                               total_clusters_,
                               minimum_cluster_index_, stop_coarsening_, partition_cuts_[0],
                         cluster_weights_->data(), oartition_vector_->data());
