...
ParaRestrCoarsener::ParaRestrCoarsener(int rank, int nProcs, int nParts, ostream &out)
...
void ParaRestrCoarsener::loadHyperGraph(const ParaHypergraph &h, MPI_Comm comm)
{
  int i;
  int endOffset;
  int startOffset;
...
#  endif
  ...
  MPI_Allgather(&minVertexIndex, 1, MPI_INT, minLocIndices.getArray(), 1, MPI_INT, comm);
  MPI_Allgather(&maxVertexIndex, 1, MPI_INT, maxLocIndices.getArray(), 1, MPI_INT, comm);
  
  for (i=0;i<numProcs;++i)
    procs[i] = i;

  CompleteBinaryTree<int> vToProc(procs.getArray(), minLocIndices.getArray(),numProcs);
...
#  endif		      
		    ...
		    {		  
		      ...
		      for (l=startOffset;l<endOffset;++l)
			{
			  if(localPins[l]>=minLocIndices[j] && localPins[l]<maxLocIndices[j])
			    dataOutSets[j]->assign(sendLens[j]++, localPins[l]-minLocIndices[j]);
			}
		    }
...
ParaHypergraph *ParaRestrCoarsener::contractHyperedges(ParaHypergraph &h, MPI_Comm comm) 
{
  ParaHypergraph *coarseGraph = new ParaHypergraph(myRank,numProcs,clusterIndex,totalClusters,myMinCluIndex,stopCoarsening,partitionCuts[0],clusterWeights->getArray(),pVector->getArray());
