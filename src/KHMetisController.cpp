...
KHMetisController::KHMetisController(GreedyKwayRefiner* k, int rank, int nProcs, int nParts, const int *options, ostream &out)
...
void KHMetisController::runSeqPartitioner(ParaHypergraph &hgraph, MPI_Comm comm)
{
...
#  endif
  ...
  HMETIS_PartKway(numVertices,numHedges,vWeights,hOffsets,pinList,hEdgeWts,numParts,ubFactor,khMetisOptions.getArray(),pArray,&numHedgesCut);
