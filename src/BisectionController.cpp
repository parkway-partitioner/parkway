...
BisectionController::BisectionController(int nRuns, double kT, double redFactor, int eeP, int percentile, int inc, int dispL, ostream &out)
  : out_stream(out)
{
  numSeqRuns = nRuns;
  eeParam = eeP;
...
void BisectionController::buildCoarsener(double redRatio, int cType, int minNodes)
{
  switch(cType)
    {
    case FCwithFanOutDiv:
      coarsener = new FCCoarsener(Shiftl(minNodes,1),-1,redRatio,1,1,dispLevel);
      break;

    case FCwithoutFanOutDiv:
      coarsener = new FCCoarsener(Shiftl(minNodes,1),-1,redRatio,0,1,dispLevel);
      break;

    case FCwithFanOut:
      coarsener = new FCCoarsener(Shiftl(minNodes,1),-1,redRatio,1,0,dispLevel);
      break;
 
    case FCwithoutFanOut:
      coarsener = new FCCoarsener(Shiftl(minNodes,1),-1,redRatio,0,0,dispLevel);
      break;
...
void BisectionController::computeBisection()
{
  ...
  for (i=0;i<numSeqRuns;++i)
	...
	{
	  bestCut = cut;
	  coarseGraph->copyOutPartition(bestPartition.getArray(),numOrigVertices,0);
	}
  ...
  origGraph->copyInPartition(bestPartition.getArray(),numOrigVertices,0,bestCut);
}
...
#  endif
  ...
  maxVertWt = static_cast<int>(floor(static_cast<double>(maxPartWt)-avePartWt));
