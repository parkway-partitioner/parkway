...
void loader::compute_hyperedges_to_load(ds::bit_field &to_load,
  ...
  double average_length = static_cast<double>(hypergraph_data[0]) /
                                              hypergraph_data[1];
  ...
  info("-- Percentile:               %i\n"
       "-- Maximum hyperedge length: %i\n"
       "-- Average hyperedge length: %i\n"
       "-- Percentile length:        %i\n",
       percentile_, max_length, average_length, percentile_length);
