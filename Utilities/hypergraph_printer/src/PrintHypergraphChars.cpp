...
int main(int argc, char **argv)
{
  if(argc < 2)
    {
       cout << "\nUSAGE:" << endl << endl
	    << "% " << argv[0] << " [options] <hypergraph filename>" << endl << endl       
	    << "PARAMETERS:" << endl << endl
	    << "\t -ptype <type_format>" << endl
	    << "\t    - integer specifying how statistics are calculated:" << endl
	    << "\t      0 -> calculate the statistics on the hypergraph as on file" << endl
	    << "\t      1 -> ignore hyperedges of length one" << endl << endl
	    << "\t -otype <type_format>" << endl
	    << "\t    - string specifying the output file. If not specified then" << endl
	    << "\t      defaults output to screen" << endl 
	    << "\t -nz <0 or 1>" << endl
	    << "\t    - specifies whether to output info about non-zeros on main" << endl
	    << "\t      diagonal - 1 yes, 0 - no" << endl << endl;
       exit(1);
    }
  
  char *out_file = StringUtils::getParameterAsCharPtr(argc, argv, "-otype", NULL);
  ...
  if(out_file)
    {
      ...
      if(!output->good())
	{
	  cout << "could not open file " << out_file
	       << " - exit" << endl;	 
	  exit(0);
	}
    }
...
{  
  ...
  cout << "AS ON FILE: numVertices = " << numVertices
       << ", numHedges = " << preamble[0]
       << ", numPins = " << numPins << endl;
  ...
  if(removeSingletons)
	...
	{	 	 
	  ...
	  in_stream.read((char*)(hEdgeData.getArray()),sizeof(int)*hEdgeDataLength);
...
void HypergraphChars::outputChars(ostream &out)
{
  ...
  out << "hyperedge length percentiles: (weighted ave, 25, 50, 75, 95, maxLength) " << endl;  
  out << "\t" << weighted_ave/j << " ";
...
void HypergraphChars::qsort(const int left, const int right, int *array, const int *cmp)
{
