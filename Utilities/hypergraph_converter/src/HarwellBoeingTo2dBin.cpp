...
#  endif
  ...
  cout << "numVertices = " << num2Dvertices 
       << ", numHedges = " << num2Dhedges
...
#  endif
  ...
  for (i=0; i<num2Dvertices; ++i){
    if(hasZeroWeight(i) == 1) vWeights[i] = 0;
    else vWeights[i] = 1;
#  ifdef PRINT_GRAPH
...
#  endif
  ...
  out_stream.open(zeroVertsFile, ofstream::out | ofstream::app | ofstream::binary);
  ...
  out_stream.write((char*)(zeroWeightVerts.getArray()), sizeof(int)*numZeroWeightVerts);
