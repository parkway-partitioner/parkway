...
HMeTiS2Bin::HMeTiS2Bin()
  : HypergraphTextFileReader(), TextToBinConverter() 
{
...
void HMeTiS2Bin::convert(const char *filename)
{
  ...
  while(!in_stream.eof() && numHedgesRead < numHedges)
	...
	{
	  setLengthParameter();
	  out_stream.write((char*)(hEdgeData.getArray()), sizeof(int)*dataLength);
	  numPinsRead = 0;
	}	 
    }
...
int HMeTiS2Bin::computeNumPins(ifstream &in_stream)
{
  ...
  while(!in_stream.eof() && numHedgesRead < numHedges)
    { 
      getLine(in_stream);
      numHedgesRead += countPinsInLine(buffer.getArray(),numPins);
    }
