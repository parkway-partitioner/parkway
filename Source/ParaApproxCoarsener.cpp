...
ParaApproxCoarsener::ParaApproxCoarsener(int _rank, int _numProcs, int _numParts, int percentile, int inc, ostream &out)
...
void ParaApproxCoarsener::loadHyperGraph(const ParaHypergraph &h, MPI_Comm comm)
{
  int i;
  int vertsPerProc;
  int endOffset;
  ...
  if(currPercentile < 100)
    computeHedgesToLoad(toLoad,numLocalHedges,localHedgeWeights,localHedgeOffsets, comm);
  else
    toLoad.set1();
...
void ParaApproxCoarsener::computeHedgesToLoad(BitField &toLoad, int numH, int *hEdgeWts, int *hEdgeOffsets, MPI_Comm comm)
{
  register int i;
  register int j;
  ...
  Funct::qsortByAnotherArray(0,numH-1,hEdges.getArray(),hEdgeLens.getArray(),INC);
  toLoad.set1();
