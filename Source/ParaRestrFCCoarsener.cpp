...
ParaRestrFCCoarsener::ParaRestrFCCoarsener(int rank, int nProcs, int nParts, int verVisOrder, int divByWt, int divByLen, ostream &out)
...
void ParaRestrFCCoarsener::buildAuxiliaryStructs(int numPins, double aveVertDeg, double aveHedgeSize)
{
...
ParaHypergraph *ParaRestrFCCoarsener::coarsen(ParaHypergraph &h, MPI_Comm comm)
{
...
#  endif
  ...
  limitOnIndexDuringCoarsening = numLocalVertices - static_cast<int>(floor(metricVal-1.0));
  clusterIndex = 0;
...
#  endif	
		  ...
		  if(neighVertex != vertex && partitionVector[neighVertex] == vertexPart)
		    {
		      // ###
			...
			{		
			  if(matchVector[neighVertex] == -1)
			    neighPairWts.assign(numNeighbours, vWeight[vertex]+vWeight[neighVertex]);
			  else
			    neighPairWts.assign(numNeighbours, vWeight[vertex]+(*clusterWeights)[matchVector[neighVertex]]);
			  
			  neighVerts.assign(numNeighbours, neighVertex);

			  if(divByHedgeLen)
			    connectVals.assign(numNeighbours, static_cast<double>(hEdgeWt)/(hEdgeLen-1));  
			  else
...
#  endif
			  if(divByHedgeLen)
			    connectVals[neighVertexEntry] += (static_cast<double>(hEdgeWt)/(hEdgeLen-1));
			  else
...
#  endif
	  ...
	  if(index > limitOnIndexDuringCoarsening)
	    {
	      reducedBy = static_cast<double>(numLocalVertices) / (numNotMatched+clusterIndex);
