...
VCycleAllBisectionController::VCycleAllBisectionController(int nRuns, double kT, double redFactor, int eeParam, int percentile, int inc, int dispL, ostream &out)
  : VCycleBisectionController(nRuns,kT,redFactor,eeParam,percentile,inc,dispL,out)
{
...
void VCycleAllBisectionController::computeBisection()
{
  ...
  for (i=0;i<numSeqRuns;++i)
      ...
      while(coarseGraph != origGraph)
	{
	  ...
	  recordVCyclePartition(finerGraph->getPartVectorArray(),finerGraph->getNumVertices());
...
#  endif
		...
		{
		  recordVCyclePartition(coarseGraph->getPartVectorArray(),coarseGraph->getNumVertices());
	  ...
	  coarseGraph->copyInPartition(vCyclePartition.getArray(),coarseGraph->getNumVertices(),0,firstCutSize); 
	}
#  ifdef DEBUG_CONTROLLER
