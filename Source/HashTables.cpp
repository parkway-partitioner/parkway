...
template <class T>
MapFromPosInt<T>::MapFromPosInt(int _size)
{
  createTable(_size);
}
...
template <class T>
void MapFromPosInt<T>::createTable(int _size)
{  
  numEntries = 0;
...
template <class T>
void MapFromPosInt<T>::destroyTable()
{
  numEntries = 0;
  table.setLength(0);
  keys.setLength(0);
...
template <class T>
void MapFromPosInt<T>::recoverTable()
{
  table.setLength(size);
  keys.setLength(size);
  numEntries = 0;
...
template <class T>
int MapFromPosInt<T>::insertKey(int key, T val)
{
...
template <class T>
T &MapFromPosInt<T>::getVal(int key)
{
  register int indepKey = TableUtils::scatterKey(key);
  register int slot = PRIMARY_HASH(indepKey,size);
...
NewHedgeIndexTable::NewHedgeIndexTable(int _size)
{
  createTable(_size);
}
...
int MatchRequestTable::lookupClusterWt(register int _vertex) const
{
  ...
  if(entry) return entry->getClusterWt();
  else return -1;
}
...
int MatchRequestTable::lookupCluIndex(register int _vertex) const
{
  ...
  if(entry) return entry->getCluIndex();
  else return -1;
}
...
int MatchRequestTable::lookupNumLocals(register int _vertex) const
{
  ...
  if(entry) return entry->getNumLocals();
  else return -1;
}
...
void MatchRequestTable::addLocal(int _vertex, int _local, int locWt, int proc)
{
  ...
  if(entry) 
    ...
    {
      MatchRequestEntry *newEntry = new MatchRequestEntry(_vertex,_local,locWt,proc,table[slot]);
      table[slot] = newEntry;
...
void MatchRequestTable::removeLocal(register int vertex, int locVertex, int locWt) 
{
  register MatchRequestEntry *entry = table[Mod(vertex,size)];
