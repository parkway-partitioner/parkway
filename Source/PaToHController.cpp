...
PaToHController::PaToHController(GreedyKwayRefiner* k, int rank, int nProcs, int nParts, int setting, ostream &out)
...
#  endif
  ...
  PaToH_Alloc(&args, numVertices, numHedges, _nconst, vWeights, hEdgeWts, hOffsets, pinList);
  ...
  for (i=0;i<numLocalRuns;++i)
    {
      PaToH_Partition(&args, numVertices, numHedges, vWeights, hEdgeWts, hOffsets, pinList, pArray, pWeights.getArray(), &cut);
