...
RestrFCCoarsener::RestrFCCoarsener(int _min, int _maxwt, double r, int fanOut, int dbWt, int dL)
...
{  
  ...
  FastDynaArray<int> *coarsePVector = new FastDynaArray<int>;
...
#  endif
  ...
  for (; i<numVertices;++i)
      ...
      if(matchVector[vertices[i]] == -1)
	{
		  ...
		  for (ij=hEdgeOffsets[hEdge];ij<endHedgeOffset;++ij)
		      ...
		      if(candVertex != v && partitionVectors[candVertex] == vPart) 
			{
			  ...
			  if(candVertexEntry == -1) 
				...
				neighPairWts.assign(numNeighbours,vWeight[v]+vWeight[candVertex]);
			      else 
				neighPairWts.assign(numNeighbours,vWeight[v]+(*coarseWts)[matchVector[candVertex]]);
			      
			      if(utilFanOut)
				connectVals.assign(numNeighbours,static_cast<double>(hEdgeWeight[hEdge])/(endHedgeOffset-(hEdgeOffsets[hEdge]+1)));
			      else
				connectVals.assign(numNeighbours,static_cast<double>(hEdgeWeight[hEdge]));
			    ...
			    {
			      if(utilFanOut)
				connectVals[candVertexEntry] += static_cast<double>(hEdgeWeight[hEdge])/(endHedgeOffset-(hEdgeOffsets[hEdge]+1));
			      else
				connectVals[candVertexEntry] += static_cast<double>(hEdgeWeight[hEdge]);
			    }
			}
		    }
		}
	  ...
	  if(i > limitOnIndex)
	    {
	      reducedBy = static_cast<double>(numVertices) / (numNotMatched+coarseIndex);
...
#  endif
  ...
  return (buildCoarseHypergraph(coarseWts->getArray(), coarsePVector->getArray(), coarseIndex, h.getTotWeight()));
}
