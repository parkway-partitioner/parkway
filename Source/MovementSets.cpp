...
MovementSetTable::MovementSetTable(int nParts, int nProcs)
{
  ...
  for (i=0;i<setArrayLen;++i) 
	...
	{      
	  sets[i] = new FastDynaArray<MOVE_SET>(numProcs);
  ...
  for (i=0;i<numProcs;++i)    
    restoringMoves[i] = new FastDynaArray<int>(256);    
}
...
MovementSetTable::~MovementSetTable()
{
  ...
  for (i=0;i<numProcs;++i)    
    DynaMem<FastDynaArray<int> >::deletePtr(restoringMoves[i]);
}
...
void MovementSetTable::completeProcSets(int proc, int dataLen, const int *data)
{ 
  register int i = 0;
  register int wt;
