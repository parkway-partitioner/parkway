...
RecurBisectController::RecurBisectController(BisectionController *b, GreedyKwayRefiner *k, int rank, int nProcs, int nParts, int nBisectRuns, ostream &out)
...
#  endif
  ...
  if(numSeqRuns <= numProcs)
    {
      if(myRank < numSeqRuns) numMyPartitions = 1;
      else numMyPartitions = 0;
    }
  else
    {
      ...
      if(myRank < i) numMyPartitions = j + 1;
      else numMyPartitions = j;
    }
...
void RecurBisectController::runSeqPartitioner(ParaHypergraph &hgraph, MPI_Comm comm)
{
...
#  endif
      ...
      MPI_Gather(&locVertPartInfoLen, 1, MPI_INT, recvLens.getArray(), 1, MPI_INT, destProcessor, comm);
...
#  endif       
      ...
      MPI_Gatherv(locVertPartitionInfo.getArray(), locVertPartInfoLen, MPI_INT, allPartitionInfo.getArray(), recvLens.getArray(), recvDispls.getArray(), MPI_INT, destProcessor, comm);
...
void RecurBisectController::initSeqPartitions(ParaHypergraph &hgraph, MPI_Comm comm)
{
  register int i;
  register int j;
  register int ij;
...
#  endif
  ...
  MPI_Alltoallv(sendArray.getArray(), sendLens.getArray(), sendDispls.getArray(), MPI_INT, hGraphPartitionVector, recvLens.getArray(), recvDispls.getArray(), MPI_INT, comm);

  // ###
  // communicate partition cuts
  // ###
  
  MPI_Allgather(&numMyPartitions, 1, MPI_INT, recvLens.getArray(), 1, MPI_INT, comm);
  ...
  MPI_Allgatherv(hPartitionCutsArray, numMyPartitions, MPI_INT, hGraphPartCuts, recvLens.getArray(), recvDispls.getArray(), MPI_INT, comm);
...
void RecurBisectController::recursivelyBisect(const Bisection &b, MPI_Comm comm)
{  
  int cut;
  int rank;
  int nProcs;
  ...
  if(nProcs == 1)
    {
      ...
      if(bisectAgain == 1) bisector->bisect(h,maxPartWt);	
      else bisector->bisect(h,computeMaxWt(logK-bisectAgain));
	...
	{	  
	  ...
	  for (i=0;i<numVerts;++i)
		...
		locVertPartitionInfo.assign(locVertPartInfoLen++,(bisectionPart | (1<<(logK-1))));	   		  		
	    }	      
	}
      else
	{
	...
	{
	  MPI_Bcast(h->getPartVectorArray(),h->getNumVertices(),MPI_INT, bestCutProc, comm);
...
void RecurBisectController::splitBisection(const Bisection &b, Bisection* &newB, MPI_Comm comm) const
{
  int rank;
  int nProcs;
...
#  endif
      ...
      newB = new Bisection(newH,bisectAgain-1,Or(bPartID,Shiftl(1,(logK-bisectAgain))));
...
void RecurBisectController::splitBisection(const Bisection &b, Bisection* &l, Bisection* &r) const
{
  register int i;
  register int j;
...
#  endif
  ...
  for (i=0;i<numHHedges;++i)
      ...
      for (j=hHedgeOffsets[i];j<endOffset;++j)
	{
	  ...
	  if(hPartVector[v] == 0)
	    ...
	    {
	      rightPinList->assign(numRightPins+(rightHedgeLen++),mapFromHtoNewH[v]);
	    }
	}
  ...
  rightH->setHedgeOffsetArray(rightHedgeOffsets->getArray(),numRightHedges+1);
  ...
  r = new Bisection(rightH,bisectAgain-1,Or(bPartID,Shiftl(1,(logK-bisectAgain))));
...
double RecurBisectController::recursivelyComputeMax(double currAve, int depth) const
{
  if(depth == 0) return (currAve+currAve*bisectConstraint);
  return (recursivelyComputeMax((currAve+currAve*bisectConstraint)/2, depth-1));
}
