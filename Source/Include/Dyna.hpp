...
class FastDynaArray {
 protected:
  ...
  inline int expand(register int index) {
    ...
    if (!adjust(targetLength - length)) return 0;
    return 1;
  }
 ...
 public:
  ...
  inline int adjust(register int size) {
    ...
    if (newLength <= 0) {
      if (length) DynaMem<T>::deleteArr(array);
    ...
    if (size >= 0)
      for (i = 0; i < length; ++i) extendedArray[i] = array[i];
    else
      for (i = 0; i < newLength; ++i) extendedArray[i] = array[i];
