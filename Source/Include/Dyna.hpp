...
protected:
  ...
  inline int expand(register int index) 
  {
    ...
    while (index >= targetLength) targetLength <<= 1;
...
#  ifdef DEBUG_BASICS
	char message[512];
	sprintf(message, "FastDynaArray: (%d): memory allocation (size %d) failed!\n", (int)this, size); 
	cout << message;
#  endif
	abort();
      }
  ...
  {    
    ...
    if (!adjust(size - length)) return 0;
    length = size;
    return 1;
  }
...
#  ifdef DEBUG_BASICS
	char message[512];
	sprintf(message, "FastDynaArray (%d): Array subscript [%d] out of range (Valid range is 0 to %d)", (int)this, index, length-1);
	cout << message;
	delayT();
#  endif
	abort();
