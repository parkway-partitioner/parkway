...
extern "C" {
  void HMETIS_PartKway(int nv, int nh, int *vwt, int *hptr, int *hv, int *hwt, int p, int ub, int *opt, int *pvector, int *cut); 
}
...
public:
  ...
  KHMetisController(GreedyKwayRefiner* k, int rank, int nProcs, int nParts, const int *options, ostream &out);
  ~KHMetisController();
