...
WebGraphSeqController::WebGraphSeqController(int rank, int nProcs, int nParts, ostream &out)
...
void WebGraphSeqController::initCoarsestHypergraph(ParaHypergraph &hgraph, MPI_Comm comm)
{
  /***********************
...
vertices.
  ...
  ************************/
  ...
  MPI_Allgather(&numLocalVertices, 1, MPI_INT, recvLens.getArray(), 1, MPI_INT, comm);
...
#  endif
  ...
  MPI_Allgatherv(localVertWeight,numLocalVertices,MPI_INT,vWeights->getArray(),recvLens.getArray(),recvDispls.getArray(),MPI_INT,comm);  
  MPI_Allgather(&numLocalHedges, 1, MPI_INT, recvLens.getArray(), 1, MPI_INT, comm);
  ...
  MPI_Allgatherv(localHedgeWeights,numLocalHedges,MPI_INT,hEdgeWeights->getArray(),recvLens.getArray(),recvDispls.getArray(),MPI_INT,comm);  
  ...
  MPI_Allgatherv(localHedgeOffsets,numLocalHedges+1,MPI_INT,recvArray.getArray(),recvLens.getArray(),recvDispls.getArray(),MPI_INT,comm);  
...
#  endif
  ...
  MPI_Allgatherv(localPins,numLocalPins,MPI_INT,pinList->getArray(),recvLens.getArray(),recvDispls.getArray(),MPI_INT,comm);  
...
void WebGraphSeqController::initSeqPartitions(ParaHypergraph &hgraph, MPI_Comm comm) 
{ 
  register int i;
  register int j;
  register int ij;
  ...
  MPI_Gather(&myBestCut, 1, MPI_INT, procCuts.getArray(), 1, MPI_INT, ROOT_PROC, comm);
  ...
  if(myRank == ROOT_PROC)
      ...
      for (i=0;i<numProcs;++i)
	{
	  ...
	  else keepPartitions[proc] = 0;	    
	}
    }

  MPI_Scatter(keepPartitions.getArray(), 1, MPI_INT, &keepMyPartition, 1, MPI_INT, ROOT_PROC, comm);
...
#  endif
  ...
  MPI_Alltoallv(sendArray.getArray(), sendLens.getArray(), sendDispls.getArray(), MPI_INT, hPartitionVector, recvLens.getArray(), recvDispls.getArray(), MPI_INT, comm);

  // ###
  // communicate partition cuts
  // ###
  
  MPI_Allgather(&keepMyPartition, 1, MPI_INT, recvLens.getArray(), 1, MPI_INT, comm);
  ...
  MPI_Allgatherv(pCuts, keepMyPartition, MPI_INT, hPartCuts, recvLens.getArray(), recvDispls.getArray(), MPI_INT, comm);
