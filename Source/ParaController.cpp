...
ParaController::ParaController(ParaCoarsener& c, ParaRefiner& r, SeqController& con, int rank, int nP, int percentile, int inc, int approxRef, ostream &out)
  : GlobalCommunicator(rank,nP), out_stream(out), coarsener(c), refiner(r), seqController(con)
{
  shuffled = 0;
  numParaRuns = -1;
  numTotalParts = -1;
...
void ParaController::setPrescribedPartition(const char *filename, MPI_Comm comm)
{
...
void ParaController::storeBestPartition(int numV, const int *array, MPI_Comm comm)
{
...
void ParaController::partitionToFile(const char *filename, MPI_Comm comm) const
{
  register int i;
  ...
  for (i=0;i<numProcs;++i)
      ...
      if(myRank == i)
	{
	  ...
	  if(!out.is_open())
	    ...
	    out.write((char*)(bestPartition.getArray()),sizeof(int)*numOrigLocVerts);	  
...
void ParaController::copyOutPartition(register int numVertices, register int *pVector) const
{
  register int i;
...
{
  //write_log(myRank, "HedgeIndex diff: %d", HedgeIndexEntry::getNumAllocated()-HedgeIndexEntry::getNumDeleted());
  //write_log(myRank, "MatchEntry diff: %d", MatchEntry::getNumAllocated()-MatchEntry::getNumDeleted());
  write_log(myRank, "MatchRequestEntry diff: %d", MatchRequestEntry::getNumAllocated()-MatchRequestEntry::getNumDeleted());
  //write_log(myRank, "IndexEntry diff: %d", IndexEntry::getNumAllocated()-IndexEntry::getNumDeleted());
  //write_log(myRank, "ConnVertData diff: %d", ConnVertData::getNumAllocated()-ConnVertData::getNumDeleted());
  //write_log(myRank, "VertexPartEntry diff: %d", VertexPartEntry::getNumAllocated()-VertexPartEntry::getNumDeleted());
  //write_log(myRank, "DuplRemEntry diff: %d", DuplRemEntry::getNumAllocated()-DuplRemEntry::getNumDeleted());
  //write_log(myRank, "PVectorEntry diff: %d", PVectorEntry::getNumAllocated()-PVectorEntry::getNumDeleted());
}
#  endif
