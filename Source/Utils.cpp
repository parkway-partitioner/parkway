...
ParaCoarsener *Utils::buildParaCoarsener(int my_rank, int num_proc, int num_parts, double constraint, ParaHypergraph *h, ostream &out, const int *options, MPI_Comm comm)
{
  ...
  if(coarsener_type == ParaFCC)
	...
	default:
      ...
      c = new ParaFCCoarsener(my_rank,num_proc,num_parts,vertexVisitOrder,matchReqVisitOrder,divByCluWt,divByHedgeLen,out);
  ...
  if(coarsener_type == Para2DModel)
	...
	default:
      ...
      c = new Para2DModelCoarsener(my_rank,num_proc,num_parts,vertexVisitOrder,matchReqVisitOrder,divByCluWt,divByHedgeLen,out);
...
ParaRestrCoarsener *Utils::buildParaRestrCoarsener(int my_rank, int num_proc, int num_parts, double constraint, ParaHypergraph *h, ostream &out, const int *options, MPI_Comm comm)
{
  ...
  if(coarsener_type == ParaRestFCC)
	...
	default:
      ...
      c = new ParaRestrFCCoarsener(my_rank, num_proc, num_parts, vertexVisitOrder, divByCluWt, divByHedgeLen, out);
...
ParaRefiner *Utils::buildParaRefiner(int my_rank, int num_proc, int num_parts, double constraint, ParaHypergraph *h, ostream &out, const int *options, MPI_Comm comm)
{
  ...
  if(refiner_type == ParaGreedyKway)
	...
	earlyExit = 1;
      ...
      r = new ParaGreedyKwayRefiner(my_rank, num_proc, num_parts, numTotPins/num_proc, earlyExit, eeLimit, out);
...
SeqController *Utils::buildSeqController(int my_rank, int num_proc, int num_parts, double constraint, ostream &out, const int *options)
{
  ...
  if(seqControllerType == RecurBisect)
    {
      ...
      BisectionController *bC = new BisectionController(numBisectRuns,keepT,redFactor,eeParam,startPercentile,inc,disp_option,out);
      ...
      GreedyKwayRefiner *k = new GreedyKwayRefiner(-1,num_parts,-1,kWayLimit,disp_option);
      ...
      seqC = new RecurBisectController(bC,k,my_rank,num_proc,num_parts,numBisectRuns,out);
  ...
  if(seqControllerType == VCycleAllRecurBisect)
    {
      ...
      VCycleAllBisectionController *bC = new VCycleAllBisectionController(numBisectRuns,keepT,redFactor,eeParam,startPercentile,inc,disp_option,out);
  ...
  if(seqControllerType == VCycleFinalRecurBisect)
    {
      ...
      VCycleFinalBisectionController *bC = new VCycleFinalBisectionController(numBisectRuns,keepT,redFactor,eeParam,startPercentile,inc,disp_option,out);
  ...
  if(seqControllerType == KhMeTiS)
    {
      ...
      seqC = new KHMetisController(k,my_rank,num_proc,num_parts,seq_options.getArray(),out);
  ...
  if(seqControllerType == PaToH)
    {
      ...
      seqC = new PaToHController(k,my_rank,num_proc,num_parts,options[21],out);
...
ParaController *Utils::buildParaController(int my_rank, int num_proc, int num_parts, int num_tot_verts, double constraint, ParaCoarsener *c, ParaRestrCoarsener *rc, ParaRefiner *r, SeqController *s, ostream &out, const int *options, MPI_Comm comm)
{
  ...
  if(paraControllerType == BasicParaC)    
    paraC = new BasicParaController(*c,*r,*s,my_rank,num_proc,percentile,perCentInc,approxRef,out);          
  ...
  if(paraControllerType == ParaVCycleBig)
      ...
      if(!rc || noCoarsening)
	{
	  ...
	  if(!rc && my_rank == 0) 
	    {
	      sprintf(message, "p[%d] no ParaRestrCoarsener - create basic controller\n", my_rank);	  	  
	      cout << message;
	    }
	  
	  if(noCoarsening && my_rank == 0)
	    {
	      sprintf(message, "p[%d] max coarse hypergraph size > orig hypergraph size - no v-cycles possible\n", my_rank);
	      cout << message;
	    }
      ...
      else
	{
	  ...
	  paraC = new ParaVCycleFinalController(*rc,*c,*r,*s,my_rank,num_proc,percentile,perCentInc,approxRef,limitOnCycles,limitAsPercent,out);	  
	}
    }
  ...
  if(paraControllerType == ParaVCycleAll)
      ...
      else
	{
	  ...
	  paraC = new ParaVCycleAllController(*rc,*c,*r,*s,my_rank,num_proc,percentile,perCentInc,approxRef,limitOnCycles,limitAsPercent,out);	  
	}
    }
...
void Utils::initDefaultValues(const int *userOptions, int *programOptions)
{
  ...
  if(programOptions[0] == 0) 
  ...
  else 
    {
      ...
      if(userOptions[1] >= 0) programOptions[1] = userOptions[1];
      else programOptions[1] = 0;
...
void Utils::checkPartsAndProcs(int num_parts, int num_procs, int seqOption, int paraOption, ostream &out, MPI_Comm comm)
{
  int my_rank;
  ...
  if(paraOption > 1)
      ...
      if(!Funct::isPowerOf2(num_procs))
	{
	  MPI_Barrier(comm);
	  if(my_rank == 0)
	    out << "number of processors must be a power of two when using parallel V-Cycling - abort\n";
      ...
      if(num_parts < num_procs || num_parts%num_procs > 0)
	{
	  MPI_Barrier(comm);
	  if(my_rank == 0)
	    out << "number of parts in partition must be divisible by number of processors when using parallel V-Cycling - abort\n";
      ...
      if(seqOption <= 3 && !Funct::isPowerOf2(num_parts))
	{
	  MPI_Barrier(comm);
	  if(my_rank == 0)
	    out << "number of parts in partition must be a power of two when using generic recursive bisection - abort\n";
      ...
      if(seqOption <= 3 && !Funct::isPowerOf2(num_parts))
	{
	  MPI_Barrier(comm);
	  if(my_rank == 0)
	    out << "number of parts must be a power of two when using generic recursive bisection - abort\n";
	  MPI_Abort(comm,0);
	}
    }
}
