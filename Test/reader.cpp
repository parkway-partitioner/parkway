...
void initGraphStructs(int &numLocalVertices, int &numLocalHedges, int* &vWeights, int* &hEdgeWts, int* &pinList, int* &offsets, const char *filename, int myRank)
{
  int buffer[3];
  int hEdgeDataLen;
  ...
  if(!vWeights) return (error(myRank, numLocalVertices, "[loc vertices]"));
...
void testRecordedPartition(const char *filename, int myRank, int numProcs, int numParts, double constraint, ostream &out, MPI_Comm comm)
{
  ParaHypergraph *h = new ParaHypergraph(myRank,numProcs,filename,1,out,comm);
...
void testRecordedPartition(const char *filename, const int *pVector, int numLocVerts, int myRank, int numProcs, int numParts, double constraint, ostream &out, MPI_Comm comm)
{
  ParaHypergraph *h = new ParaHypergraph(myRank,numProcs,filename,1,out,comm);
